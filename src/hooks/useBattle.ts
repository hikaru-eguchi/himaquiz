...
export const useBattle = (playerName: string) => {
  ...
  const [gameType, setGameType] = useState<"quiz" | "dungeon" | "dobon" | "majority">("quiz");
  ...
  useEffect(() => {
    ...
    const handleRoomPlayersUpdate = ({
      ...
      gameType: "quiz" | "dungeon" | "dobon" | "majority";
      ...
      if (gameType === "dungeon" || gameType === "dobon" || gameType === "majority") {
  ...
  /* =========================
     参加処理
  ========================= */
  const joinRandom = (options?: { maxPlayers?: number; gameType?: "quiz" | "dungeon" | "dobon" | "majority" }, onJoined?: (code: string) => void) => {
    if (!socket) return;
  ...
  const joinWithCode = (code: string, count: string, type: "quiz" | "dungeon" | "dobon" | "majority") => {
    if (!socket) {
