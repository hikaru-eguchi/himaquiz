...
type WordPlayer = {
  ...
  correctCount?: number;
  eliminatedReason?: "wrong_area" | "timeout" | "unknown";
  invincibleUntil?: number;
  speedBoostUntil?: number;
  invisibleUntil?: number;
  barrierUntil?: number;
  barrierCount?: number;
  activeItem?: "speed" | "invisible" | "barrier" | null;
  itemEffect?: "speed" | "invisible" | "barrier" | null;
  isOni?: boolean;
  role?: "oni" | "runner";
  oniTimeMs?: number;
  lastOniStartedAt?: number;
  eliminatedAt?: number;
  userId?: string | null;
...
type WordBookItem = {
  id: string;
  type: "letter" | "hint" | "fake" | "speed" | "invisible" | "barrier";
  x: number;
  y: number;
  solved?: boolean;
  usedBy?: string[];
...
type WordBookQuestionPayload = {
  bookId: string;
  bookType: "letter" | "hint" | "fake" | "speed" | "invisible" | "barrier";
  itemType?: "speed" | "invisible" | "barrier";
...
const PENDING_KEY = "quiz_onigokko_award_pending_v1";
...
const ONIGOKKO_ONI_IMAGE = "/images/onigokko1.png";

const getOnigokkoItemLabel = (type?: string | null) => {
  if (type === "speed" || type === "letter") return "スピードUP";
  if (type === "invisible" || type === "hint") return "透明化";
  if (type === "barrier" || type === "fake") return "バリア";
  return "？アイテム";
};

const getOnigokkoItemShortLabel = (type?: string | null) => {
  // if (type === "speed" || type === "letter") return "⚡";
  // if (type === "invisible" || type === "hint") return "👻";
  // if (type === "barrier" || type === "fake") return "🛡️";
  return "❓";
};

const getOnigokkoItemStyle = (type?: string | null) => {
  // if (type === "speed" || type === "letter") {
  //   return "border-yellow-100 bg-gradient-to-br from-yellow-300 via-orange-400 to-red-500 text-white shadow-[0_0_24px_rgba(251,191,36,0.8)]";
  // }
  // if (type === "invisible" || type === "hint") {
  //   return "border-sky-100 bg-gradient-to-br from-cyan-300 via-sky-400 to-blue-500 text-white shadow-[0_0_24px_rgba(56,189,248,0.75)]";
  // }
  // if (type === "barrier" || type === "fake") {
  //   return "border-emerald-100 bg-gradient-to-br from-emerald-300 via-lime-400 to-yellow-400 text-emerald-950 shadow-[0_0_24px_rgba(132,204,22,0.75)]";
  // }
  return "border-white bg-gradient-to-br from-red-400 via-orange-400 to-yellow-300 text-white shadow-[0_0_24px_rgba(251,146,60,0.75)]";
};

const isOniPlayer = (player: WordPlayer) =>
  player.isOni || player.role === "oni";

const getActiveOnigokkoEffects = (player: WordPlayer) => {
  const now = Date.now();
  const effects: { key: string; label: string; className: string }[] = [];

  if (
    (player.speedBoostUntil ?? 0) > now ||
    player.activeItem === "speed" ||
    player.itemEffect === "speed"
  ) {
    effects.push({
      key: "speed",
      label: "⚡スピードUP",
      className: "bg-yellow-300 text-orange-950 border-orange-600",
    });
  }
  if (
    (player.invisibleUntil ?? 0) > now ||
    player.activeItem === "invisible" ||
    player.itemEffect === "invisible"
  ) {
    effects.push({
      key: "invisible",
      label: "👻透明化",
      className: "bg-sky-200 text-sky-950 border-sky-600",
    });
  }
  if (
    (player.invincibleUntil ?? 0) > now ||
    (player.barrierUntil ?? 0) > now ||
    (player.barrierCount ?? 0) > 0 ||
    player.activeItem === "barrier" ||
    player.itemEffect === "barrier"
  ) {
    effects.push({
      key: "barrier",
      label: "🛡️バリア",
      className: "bg-emerald-200 text-emerald-950 border-emerald-600",
    });
  }

  if (isOniPlayer(player)) {
    effects.unshift({
      key: "oni",
      label: "👹鬼",
      className: "bg-red-500 text-white border-yellow-200",
    });
  }
...
//   value <= 100 ? (value / 100) * QUIZ_WORLD_WIDTH : value;

// const toWorldY = (value: number) =>
//   value <= 100 ? (value / 100) * QUIZ_WORLD_HEIGHT : value;
...
const reasonLabel = (reason?: WordPlayer["eliminatedReason"]) => {
  if (reason === "wrong_area") return "つかまった";
...
const buildRanksFromPlayers = (players: WordPlayer[]): RankRow[] => {
  return [...players]
    .map((p) => ({
      ...
      oniTimeMs: p.oniTimeMs ?? p.correctCount ?? 0,
    }))
    .sort((a, b) => {
      const aOni = a.oniTimeMs ?? a.correctCount ?? 0;
      const bOni = b.oniTimeMs ?? b.correctCount ?? 0;
      if (aOni !== bOni) return aOni - bOni;
      return b.score - a.score;
    })
    .map((p, index) => ({
      ...p,
      rank: index + 1,
      correctCount: p.oniTimeMs ?? p.correctCount ?? 0,
    }));
};
...
const ProgressBooks = ({ found, total }: { found: number; total: number }) => {
  return (
...
function WordGlassCard({
  ...
  return (
    <div
      className={`rounded-3xl border border-yellow-100/70 bg-white/25 shadow-[0_0_35px_rgba(251,146,60,0.32),0_0_80px_rgba(250,204,21,0.22)] backdrop-blur ${className}`}
    >
      {children}
    </div>
  );
    ...
    >
      ...
      {showText1 && (
        <motion.div
          ...
          className="mb-4"
        >
          <p className="mt-2 text-sm md:text-xl font-bold text-white md:text-lg">
            おにごっこ結果
          </p>
        </motion.div>
      )}
      ...
      {showText2 && (
        <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-4">
          <WordGlassCard className="p-4">
            <p className="text-xs font-bold text-white">鬼だった時間</p>
            <p className="text-3xl font-black text-amber-200">{Math.ceil(correctCount / 1000)}秒</p>
          </WordGlassCard>
          <WordGlassCard className="p-4">
            <p className="text-xs font-bold text-white">使ったアイテム</p>
            <p className="text-3xl font-black text-fuchsia-200">
              {foundLetters}
            </p>
          </WordGlassCard>
          <WordGlassCard className="p-4">
            <p className="text-xs font-bold text-white">獲得P</p>
            <p className="text-3xl font-black text-yellow-200">
              {earnedPoints}
            </p>
          </WordGlassCard>
          <WordGlassCard className="p-4">
            <p className="text-xs font-bold text-white">EXP</p>
      ...
      {showText3 && (
        ...
        >
          <p className="text-sm font-black text-white">あなたの結果</p>
              ...
              return (
                ...
                >
                  ...
                  <span className="text-sm font-bold text-white">
                    鬼 {Math.ceil(((rank.oniTimeMs ?? rank.correctCount ?? 0) as number) / 1000)}秒
                  </span>
                  {p && !p.alive && (
      ...
      {showButton && (
        <WordGlassCard className="mb-6 p-5">
          <div className="mb-3 text-base font-bold leading-relaxed text-white md:text-lg">
            <p className="text-amber-100">
              アイテム取得ボーナス：{basePoints}P
            </p>
          ...
          <p className="text-2xl font-black text-white md:text-3xl">
            今回の獲得：
            <span className="text-orange-500">{earnedPoints}P</span>
            <span className="mx-2 text-white">/</span>
            <span className="text-purple-500">{earnedExp}EXP</span>
          </p>
      ...
      {showButton && (
        <div className="mb-8 flex flex-col items-center justify-center gap-3 md:flex-row">
          ...
          {isCodeMatch ? (
            ...
            <button
              onClick={onNewMatch}
              className="w-full rounded-full bg-gradient-to-r from-yellow-300 to-orange-500 px-6 py-3 text-lg font-black text-black shadow-[0_0_25px_rgba(251,191,36,0.35)] md:w-auto"
            >
              もう一回やる！
            </button>
          )}
...
export default function QuizOnigokkoModePage() {
  ...
  const [bookQuestion, setBookQuestion] =
    useState<WordBookQuestionPayload | null>(null);
  const [bookResultMessage, setBookResultMessage] = useState<string | null>(
    null,
  );
  ...
  const theme = {
    label: "QUIZ ONIGOKKO",
    bg: "from-red-500 via-orange-300 to-yellow-200",
    panel: "from-yellow-300/55 via-orange-300/35 to-red-400/35",
  ...
  const cameraTargetX = me ? toWorldX(me.x) : QUIZ_ANSWER_X;
  const cameraTargetY = me ? toWorldY(me.y) : QUIZ_ANSWER_Y;
  const safeViewportWidth = viewportSize.width > 0 ? viewportSize.width : 900;

  const safeViewportHeight =
    viewportSize.height > 0 ? viewportSize.height : 620;
  // const cameraX = Math.min(
  //   0,
  //   Math.max(viewportSize.width - QUIZ_WORLD_WIDTH, viewportSize.width / 2 - cameraTargetX),
  // );
  // const cameraY = Math.min(
  //   0,
  //   Math.max(viewportSize.height - QUIZ_WORLD_HEIGHT, viewportSize.height / 2 - cameraTargetY),
  // );
  const cameraX = Math.min(
    0,
    Math.max(
      safeViewportWidth - QUIZ_WORLD_WIDTH,
  ...
  const cameraY = Math.min(
    0,
    Math.max(
      safeViewportHeight - QUIZ_WORLD_HEIGHT,
  ...
  const usedItemCount = me?.correctCount ?? 0;
  const foundLetters = usedItemCount;
      ...
      if (payload.points > 0) {
        const { error: logError } = await supabase
          ...
          .insert({
            ...
            reason:
              // `クイズおにごっこ獲得: 鬼時間${Math.ceil(payload.correctCount / 1000)}秒=${payload.basePoints}P` +
              `クイズおにごっこ獲得: アイテム${foundLetters}個=${payload.basePoints}P` +
      ...
      if (payload.exp > 0) {
        const { error: logError2 } = await supabase
          ...
          .insert({
            user_id: authedUserId,
            change: payload.exp,
            reason: `クイズおにごっこEXP獲得: 鬼時間${Math.ceil(payload.correctCount / 1000)}秒 → ${payload.exp}EXP`,
          });
        if (logError2)
  ...
  const sendMove = (dir: Direction, pressed: boolean) => {
    ...
    socket.emit("onigokko_move_input", {
      roomCode,
      dir,
      pressed,
    });
  ...
  const handleJoin = () => {
    ...
    if (mode === "random") {
      joinRandom(
        { maxPlayers: 4, gameType: "onigokko", userId: user?.id ?? null },
        (createdCode) => {
      ...
      joinWithCode(code, count, "onigokko", user?.id ?? null);
      setRoomCode(roomKey);
    }
  };
  ...
  const handleSubmitBookAnswer = (choiceIndex: number) => {
    ...
    socket.emit("onigokko_submit_item_answer", {
      roomCode,
      itemId: bookQuestion.bookId,
  ...
  const handleShareX = () => {
    const text = [
      "【ひまQ｜クイズチェイス❓】",
      `順位：${myRank ?? "-"}位`,
      `鬼だった時間：${myOniTimeSec}秒`,
      `使ったアイテム：${foundLetters}`,
  ...
  useEffect(() => {
    ...
    socket.on(
      "onigokko_item_question",
      (payload: WordBookQuestionPayload & { itemId?: string }) => {
        setBookQuestion({
          ...payload,
          bookId: payload.bookId ?? payload.itemId ?? "",
          bookType: payload.bookType ?? "speed",
        });
        setBookResultMessage(null);
      },
    );
    ...
    socket.on(
      "onigokko_item_result",
      (payload: { ok?: boolean; isCorrect?: boolean; message?: string; itemType?: string | null }) => {
        const ok = payload.ok ?? payload.isCorrect ?? false;
        setBookResultMessage(
          payload.message ??
            (ok
              ? `正解！${getOnigokkoItemLabel(payload.itemType)}が発動！`
              : "不正解！アイテムは発動しなかった！"),
        );
    ...
    socket.on("onigokko_match_ended", () => {
      setMatchEnded(true);
      setFinished(true);
    });
    ...
    socket.on("onigokko_rematch_available", () => {
    ...
    return () => {
      ...
      socket.off("onigokko_state");
      socket.off("onigokko_game_result");
      socket.off("onigokko_item_question");
      socket.off("onigokko_item_result");
      socket.off("onigokko_match_ended");
      socket.off("rematch_start");
      socket.off("onigokko_rematch_available");
    };
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.repeat || bookQuestion) return;
  ...
  useEffect(() => {
    ...
    // const oniSeconds = Math.ceil(correctCount / 1000);
    // const base = Math.max(0, 90 - oniSeconds) * 5;
    const base = foundLetters * 50;
    ...
    // const expEarned = Math.max(0, 90 - oniSeconds) * 2 + foundLetters * 10;
    const expEarned = foundLetters * 10 + (myRankNow === 1 ? 50 : 0);
    ...
    (async () => {
      try {
        ...
        const res = await submitGameResult(supabase, {
          game: "onigokko",
          score: Math.max(0, 90 - myOniTimeSec),
          title: null,
          firstPlace: myRankNow === 1,
          writeLog: true,
        });

        const modal = buildResultModalPayload("onigokko", res);
        if (modal) pushModal(modal);
      } catch (e) {
        console.error("[quiz_onigokko] submitGameResult error:", e);
      }
    })();
  }, [
    finished,
    ...
    return (
      <main className="overflow-hidden bg-gradient-to-b from-red-500 via-orange-300 to-yellow-200 text-white">
        ...
        <div className="mx-auto flex max-w-3xl flex-col items-center justify-center px-4 my-20 text-center">
          <p className="mb-3 rounded-full border border-white/60 bg-white/10 px-4 py-1 text-sm font-black tracking-[0.3em] text-amber-100">
            QUIZ ONIGOKKO
          </p>
          ...
          <WordGlassCard className="w-full max-w-md p-5 border border-white/60">
            ...
            <input
              ...
              maxLength={10}
              placeholder="最大10文字"
              className="mb-3 w-full rounded-2xl border border-white bg-white/10 px-4 py-3 text-center text-xl font-black text-white outline-none placeholder:text-white"
            />
    ...
    return (
      <main className="bg-gradient-to-b from-red-500 via-orange-300 to-yellow-200 px-4 py-10 text-center text-white">
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <AnimatePresence>
            {!readyToStart && showStartButton && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 18 }}
              >
                <p className="mb-4 text-lg font-bold text-white md:text-2xl">
                  準備できたら「対戦スタート！」を押そう！
                </p>
                <button
                  onClick={handleReady}
                  className="animate-pulse rounded-full border-2 border-yellow-100 bg-gradient-to-r from-red-500 via-orange-400 to-yellow-300 px-8 py-4 text-2xl font-black text-white shadow-[0_0_35px_rgba(245,158,11,0.55)] transition hover:scale-110"
                >
                  対戦スタート！
                </button>
              </motion.div>
          ...
          {readyToStart && (
            <p className="mt-4 text-2xl font-black text-amber-700 animate-pulse">
              全員の準備を待っています…
            </p>
          )}
        </div>
          ...
          <>
            <div className="mb-3 grid gap-3 md:grid-cols-[1fr_280px]">
              <section className="rounded-3xl border border-white/15 bg-black/35 p-1 md:p-3 backdrop-blur">
                ...
                <div
                  ref={mapViewportRef}
                  className={`relative mx-auto h-[60vh] md:h-[68vh] min-h-[440px] md:min-h-[520px] w-full max-w-[980px] overflow-hidden rounded-[2rem] border-4 ${theme.border} bg-orange-100 ${theme.glow}`}
                >
                  <div
                    className="absolute left-0 top-0"
                    style={{
                      width: QUIZ_WORLD_WIDTH,
                      height: QUIZ_WORLD_HEIGHT,
                      transform: `translate3d(${cameraX}px, ${cameraY}px, 0)`,
                      transition: "transform 80ms linear",
                    }}
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-yellow-100 via-orange-100 to-red-100" />
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(239,68,68,0.18)_1px,transparent_1px)] [background-size:54px_54px]" />
                    ...
                    {(gameState?.walls ?? []).map((wall) => (
                      <div
                        key={wall.id}
                        className="absolute rounded-md border-2 border-orange-200 bg-gradient-to-br from-red-300 via-orange-300 to-yellow-200 shadow-[0_0_18px_rgba(251,146,60,0.35),inset_0_2px_0_rgba(255,255,255,0.55)]"
                        style={{
                          left: `${wall.x}px`,
                          top: `${wall.y}px`,
                          width: `${wall.w}px`,
                    ...
                    {(gameState?.books ?? gameState?.items ?? [])
                      .filter(
                        (book) => !book.usedBy?.includes(mySocketId ?? ""),
                      )
                      .map((book) => {
                        const bookStyle = getOnigokkoItemStyle(book.type);
                        const icon = getOnigokkoItemShortLabel(book.type);

                        return (
                          <div
                            key={book.id}
                            className={`absolute flex h-12 w-12 items-center justify-center rounded-full border-4 text-2xl font-black md:h-16 md:w-16 md:text-3xl ${bookStyle} ${book.solved ? "opacity-45 grayscale" : ""}`}
                            style={{
                    ...
                    {displayPlayers.map((player) => {
                      ...
                      const isInvisibleToOthers =
                        (player.invisibleUntil ?? 0) > Date.now() && !isMe;
                        ...
                        >
                          ...
                          <Image
                            ...
                            className={`
                              ...
                              object-contain
                              ${isMe ? "opacity-100 drop-shadow-[0_0_20px_gold]" : "opacity-80"}
                              ${!player.alive ? "opacity-40 grayscale" : ""}
                              ${isInvisibleToOthers ? "!opacity-0" : ""}
                              ${isOniPlayer(player) ? "drop-shadow-[0_0_22px_rgba(239,68,68,0.95)]" : ""}
                            `}
                          />

                          {!isInvisibleToOthers && (
                            <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 w-24 truncate text-center text-xs font-black text-red-950 drop-shadow-[0_1px_0_rgba(255,255,255,0.8)]">
                              {isMe ? "YOU" : player.name}
                            </span>
                          )}

                          {getActiveOnigokkoEffects(player).length > 0 && (
                            <div className="absolute -top-8 left-1/2 flex -translate-x-1/2 flex-col items-center gap-1">
                              {getActiveOnigokkoEffects(player)
                                .filter((effect) => effect.key !== "invisible")
                                .slice(0, 2)
                                .map((effect) => (
                                  <span
                                    key={effect.key}
                                    className={`whitespace-nowrap rounded-full border px-2 py-0.5 text-[10px] font-black shadow ${effect.className}`}
                                  >
                                    {effect.label}
                                  </span>
                                ))}
                      ...
                      >
                        ...
                        >
                          <p className="text-sm font-black tracking-[0.25em] text-[#8b3a1f]">
                            {getOnigokkoItemLabel(
                              bookQuestion.itemType ?? bookQuestion.bookType,
                            )}
                          </p>
                          ...
                          <div className="mt-5 grid gap-3">
                            {bookQuestion.question.choices.map(
                              (choice, index) => (
                                <button
                                  key={`${choice}-${index}`}
                                  type="button"
              ...
              <aside className="rounded-3xl border border-white/15 bg-black/35 p-4 backdrop-blur">
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-white/10 p-3 text-sm font-bold text-white">
                  <p>PC：矢印キーで移動</p>
            ...
            <p className="mt-4 hidden text-center text-sm font-bold text-orange-600 md:block">
              PC操作：矢印キーで移動
            </p>
          </>
        ) : (
