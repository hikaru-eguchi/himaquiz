...
export async function generateMetadata({ params }: PageProps) {
  ...
  try {
    ...
    return {
      title: `${meta.title}｜ひらがな穴埋めクイズ｜ひまQ`,
      description:
        meta.description ??
        `${meta.title}に挑戦できるひらがな穴埋めクイズページです。ことば遊びを楽しみながら、語彙力やひらめきを鍛えられます。`,
    };
  } catch {
    return {
      title: "ひらがな穴埋めクイズ｜ひまQ",
      description:
        "ことば遊びを楽しめる、ひらがな穴埋めクイズページです。",
    };
  }
}
  ...
  return (
    <main className="container max-w-[1000px] mx-auto p-6">
      ...
      <section className="mb-8 md:mb-10 text-center">
        <div className="inline-block rounded-[28px] border-4 border-black px-5 py-4 md:px-8 md:py-5 shadow-[0_4px_0_0_#000] bg-gradient-to-r from-pink-100 via-fuchsia-100 to-purple-100">
        ...
        <p className="mt-4 text-gray-700 text-sm md:text-base max-w-3xl mx-auto leading-relaxed">
          ひらがな穴埋めクイズに挑戦できる問題ページです。空欄に入るひらがなを考えながら、
          ことば遊びを楽しく楽しめます。スキマ時間の脳トレや語彙力アップにもおすすめです。
        </p>
      </section>
      ...
      <section className="mt-12 flex flex-col items-center gap-4">
        <div className="flex justify-center gap-4 flex-wrap">
          {prevSlug ? (
            <Link href={`/quizbooks_hiragana/${prevSlug}`}>
          ...
          {nextSlug ? (
            <Link href={`/quizbooks_hiragana/${nextSlug}`}>
        ...
        <Link href="/quizbooks_hiragana">
          <button className="px-6 py-3 rounded-full border-2 border-black bg-pink-400 text-black font-bold shadow-md hover:scale-105 transition">
            一覧へ戻る
          </button>
        </Link>
      </section>
