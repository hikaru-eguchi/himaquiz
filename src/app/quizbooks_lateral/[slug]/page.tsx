...
export async function generateMetadata({ params }: PageProps) {
  ...
  try {
    ...
    return {
      title: `${meta.title}｜水平思考クイズ｜ひまQ`,
      description:
        meta.description ??
        `${meta.title}に挑戦できる水平思考クイズページです。ラテラルシンキングを使いながら、発想力やひらめきを楽しく鍛えられます。`,
    };
  } catch {
    return {
      title: "水平思考クイズ｜ひまQ",
      description:
        "ひらめきや発想力が試される、水平思考クイズページです。",
    };
  }
}
  ...
  return (
    <main className="container max-w-[1000px] mx-auto p-6">
      ...
      <section className="mb-8 md:mb-10 text-center">
        <div className="inline-block rounded-[28px] border-4 border-black px-5 py-4 md:px-8 md:py-5 shadow-[0_4px_0_0_#000] bg-gradient-to-r from-purple-100 via-indigo-100 to-blue-100">
        ...
        <p className="mt-4 text-gray-700 text-sm md:text-base max-w-3xl mx-auto leading-relaxed">
          水平思考クイズに挑戦できる問題ページです。常識にとらわれない発想で考えながら、
          ラテラルシンキングを楽しく鍛えられます。スキマ時間の脳トレやひらめき力アップにもおすすめです。
        </p>
      </section>
      ...
      <section className="mt-12 flex flex-col items-center gap-4">
        <div className="flex justify-center gap-4 flex-wrap">
          {prevSlug ? (
            <Link href={`/quizbooks_lateral/${prevSlug}`}>
          ...
          {nextSlug ? (
            <Link href={`/quizbooks_lateral/${nextSlug}`}>
        ...
        <Link href="/quizbooks_lateral">
          <button className="px-6 py-3 rounded-full border-2 border-black bg-indigo-400 text-black font-bold shadow-md hover:scale-105 transition">
            一覧へ戻る
          </button>
        </Link>
      </section>
